  ...
  // using sizers
{
  ...
  controlwin->SetSizer(button_sizer);
  controlwin->SetScrollRate(10, 10);
  controlwin->SetAutoLayout(true);

  button_sizer->Add(new wxButton(controlwin, MY_BUTTON_ID, wxT("Run")), 0, wxALL, 10);
  button_sizer->Add(new wxStaticText(controlwin, wxID_ANY, wxT("Cycles")), 0, wxTOP|wxLEFT|wxRIGHT, 10);
  spin = new wxSpinCtrl(controlwin, MY_SPINCNTRL_ID, wxString(wxT("10")));
  button_sizer->Add(spin, 0 , wxALL, 10);

  button_sizer->Add(new wxTextCtrl(controlwin, MY_TEXTCTRL_ID, wxT(""), wxDefaultPosition, wxDefaultSize, wxTE_PROCESS_ENTER), 0 , wxALL, 10);
  topsizer->Add(controlwin,  1, wxEXPAND | wxALL, 10);
