...
int main()
{
	...
	assert (test_table.nametable.size()==3);
	cout << "New size of test_table ok (3)" << endl;
	for (int i=0;i<test_table.nametable.size();i++)
	{
		test_table.writename(i);
		cout <<endl;
	}
	assert(test_table.getname(0)=="");
	...
	test_table.namelength(2); cout << endl;
	cout << "(Expected 0,69,13)"<<endl;
